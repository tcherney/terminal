...
/// Converts 24-bit RGB to the nearest 256-color palette index.
...
/// A terminal abstraction that provides methods for writing to the terminal, changing colors, and getting the terminal size.
pub const Term = struct {
    ...
    /// Turns on the terminal, enabling the alternate screen buffer and hiding the cursor.
    pub fn on(self: *Self) Error!void {
        try self.out(TERM_ON);
    }

    /// Turns off the terminal, disabling the alternate screen buffer and showing the cursor.
    ...
    /// Sets the foreground color using RGB values. The RGB values are converted to the nearest 256-color palette index before being applied.
    pub fn set_fg_color(self: *Self, r: u8, g: u8, b: u8) Error!void {
        try self.out(FG[rgb_256(r, g, b)]);
    }

    /// Writes a string to the terminal's standard output.
    pub fn out(self: *const Self, s: []const u8) Error!void {
        _ = try self.stdout.write(s);
    }

    /// Writes a string to the terminal's standard error.
    pub fn out_err(self: *const Self, s: []const u8) Error!void {
        _ = try self.stderr.write(s);
    }

    /// Writes a formatted string to the terminal's standard output.
    /// The format string and arguments are processed using `std.fmt.allocPrint`, which allocates a temporary buffer for the formatted output.
    /// The buffer is freed after use.
    ...
    /// Gets the size of the terminal in terms of rows and columns.
    /// The method handles different operating systems (Windows, Linux, and macOS) and returns an error if it fails to retrieve the size.
